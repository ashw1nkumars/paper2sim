...
Three real runs, one per domain, driven live through the UI using **free** LLM providers ([Google Gemini 3.6 Flash](backend/services/llm/google.py) and [OpenRouter / DeepSeek](backend/services/llm/openrouter.py)). Each paper's central claim was extracted from its arXiv abstract, turned into a Python simulation, executed in the sandbox, and rendered with a verdict.
...
The heavy lifting runs in a single Celery task, [`run_pipeline`](backend/services/pipeline.py), off the request path. Each stage writes its result back to the Redis job record so the UI can follow along live.

| # | Stage | Status | What happens | Code |
|---|-------|--------|--------------|------|
| 1 | **Ingest** | `ingesting` | Turn the submission into plain text. PDFs are parsed with `pypdf`; arXiv ids/URLs are resolved to *title + abstract* via the arXiv API (cached in Redis); pasted text is used as-is. Output is trimmed to 24k chars. | [`services/ingest/`](backend/services/ingest) |
| 2 | **Analyze** | `analyzing` | The LLM returns JSON: `claim`, `why_it_matters`, `simulation_plan`. Parsed defensively (first JSON object wins; falls back to raw text as the claim). | [`prompts.ANALYZE_SYSTEM`](backend/services/llm/prompts.py) |
| 3 | **Generate** | `generating` | The LLM writes one self-contained Python script constrained to an allowlisted scientific stack and an explicit output contract. The code block is extracted from the response. | [`prompts.GENERATE_SYSTEM`](backend/services/llm/prompts.py) |
| 4 | **Execute (+ repair)** | `executing` / `repairing` | The script runs in the [sandbox](#the-sandbox-security-model). On a non-zero exit, `stderr` is fed back to the LLM to fix the code, and it retries - up to `MAX_REPAIR_ATTEMPTS` times. | [`services/sandbox/executor.py`](backend/services/sandbox/executor.py) |
| 5 | **Summarize** | `summarizing` | The LLM writes a plain-language conclusion referencing the measured numbers; the verdict is derived from the script's `RESULT_JSON`. | [`prompts.SUMMARIZE_SYSTEM`](backend/services/llm/prompts.py) |
...
A submission becomes a `JobRecord` ([`models/job.py`](backend/models/job.py)) stored as JSON in Redis. Statuses progress:
...
paper2sim **executes LLM-generated code**, so [`executor.py`](backend/services/sandbox/executor.py) runs every script under layered constraints (defence in depth):
...
[`get_provider()`](backend/services/llm/base.py) selects the implementation based on `LLM_PROVIDER`:

- `auto` (default) → first configured of **Anthropic** → **Groq** → **Cerebras** → **Google** → **OpenRouter**, else the **mock**;
- `anthropic` → [`AnthropicProvider`](backend/services/llm/anthropic.py) (official `anthropic` SDK, `ANTHROPIC_MODEL`);
- `groq` → [`GroqProvider`](backend/services/llm/groq.py) (fast open models, `GROQ_MODEL`);
- `cerebras` → [`CerebrasProvider`](backend/services/llm/cerebras.py) (fast open models, `CEREBRAS_MODEL`);
- `google` → [`GoogleProvider`](backend/services/llm/google.py) (Gemini via its OpenAI-compatible endpoint, `GOOGLE_MODEL`);
- `openrouter` → [`OpenRouterProvider`](backend/services/llm/openrouter.py) (many models via one API, `OPENROUTER_MODEL`);
- `mock` → [`MockProvider`](backend/services/llm/mock.py).

Groq, Cerebras, Google, and OpenRouter all share one tiny [OpenAI-compatible client](backend/services/llm/openai_compat.py) (base URL + key + model, with 429 backoff), so adding another such provider is a few lines.
...
Redis is the backbone; it plays **four** roles ([`store.py`](backend/store.py), [`worker.py`](backend/worker.py)):
...
All settings have working defaults ([`core/config.py`](backend/core/config.py); see [`.env.example`](.env.example)). Set via environment or a gitignored `.env`.
...
- [`api.ts`](frontend/src/lib/api.ts) - a typed client using same-origin `/api` URLs.
...
paper2sim/
├── backend/
│   ├── main.py               # thin FastAPI app: mounts routers + artifacts
│   ├── worker.py             # Celery app (Redis broker + backend)
│   ├── store.py              # Redis: jobs, index, cache, rate-limit counters
│   ├── api/                  # HTTP layer, classified per purpose
│   │   ├── health.py · papers.py · jobs.py    # routers
│   │   └── deps.py           # shared dependencies (rate_limit)
│   ├── core/config.py        # pydantic-settings
│   ├── models/job.py         # JobRecord, Analysis, ExecutionResult, JobStatus
│   ├── helpers/parsing.py    # pure helpers: extract_code, parse_analysis, derive_verdict
│   ├── services/             # domain logic
│   │   ├── pipeline.py       # the ingest→…→summarize Celery task
│   │   ├── ingest/           # pdf.py · arxiv.py · __init__ (dispatch)
│   │   ├── llm/              # base · anthropic · groq · cerebras · google · openrouter · mock · prompts
│   │   └── sandbox/executor.py  # resource-limited code runner
│   ├── tests/                # api · pipeline · sandbox · ingest · extract
│   └── Dockerfile · requirements*.txt · pyproject.toml
├── frontend/
│   ├── src/
│   │   ├── App.tsx · main.tsx
│   │   ├── lib/api.ts        # typed API client + types
│   │   ├── components/       # SubmitForm · JobList · JobDetail · Scene3D
│   │   └── styles/app.css
│   └── Dockerfile · nginx.conf · vite.config.ts · package.json
├── docs/                     # demo GIFs + generated + 3D artifacts
...
- The API key is read **only** from environment/settings ([`config.py`](backend/core/config.py)). It is never logged, never written to a job record, and never returned by the API - responses expose only the provider *name* (e.g. `mock`).
...
**Add an LLM provider:** create `services/llm/openai.py` implementing `complete(system, user) -> str` and a `name`, then wire it into [`get_provider()`](backend/services/llm/base.py).

**Change what "proof" means:** edit the prompts in [`services/llm/prompts.py`](backend/services/llm/prompts.py) - e.g. require statistical tests, error bars, or a specific figure set. The output contract (figures + `RESULT_JSON`) is the only thing the executor depends on.

**Full-text papers:** extend [`services/ingest/arxiv.py`](backend/services/ingest/arxiv.py) to fetch and parse the PDF instead of just the abstract.
